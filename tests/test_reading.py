...
def test_reading_csv_file(mock_read_csv):
    ...
    result = reading_csv_file("succes.csv")
    assert result == [{"id": 650703, "date": "EXECUTED"}, {"id": 3107343, "date": "CANCELED"}]
    mock_read_csv.assert_called_once_with("succes.csv")


def test_reading_csv_file_empty_path():
    result = reading_csv_file("")
    assert result == "Путь не найден"


@patch("pandas.read_csv")
def test_reading_csv_file_error(mock_read_csv):
    mock_read_csv.side_effect = Exception("NotFound")
    result = reading_csv_file("error.csv")
